...
// const { PROVIDERS, getProviderClass } = require('../ai/factory'); // Removed reference to factory.js
const PROVIDERS = {
    'openai': {
        name: 'OpenAI',
        llmModels: ['gpt-4o', 'gpt-4o-mini', 'gpt-4', 'gpt-3.5-turbo'],
        sttModels: ['whisper-1']
    },
    'anthropic': {
        name: 'Anthropic',
        llmModels: ['claude-3-5-sonnet-20241022', 'claude-3-5-haiku-20241022', 'claude-3-opus-20240229'],
        sttModels: []
    },
    'gemini': {
        name: 'Google Gemini',
        llmModels: ['gemini-1.5-pro-latest', 'gemini-1.5-flash-latest'],
        sttModels: []
    },
    'ollama': {
        name: 'Ollama',
    ...
    'whisper': {
        name: 'Whisper',
        llmModels: [],
        sttModels: ['whisper-large-v3', 'whisper-medium', 'whisper-small', 'whisper-base', 'whisper-tiny']
    }
};
...
class ModelStateService extends EventEmitter {
    constructor() {
        ...
        // electron-store is only used for legacy data migration purposes.
        this.store = new Store({ name: 'oacoder-model-state' });
    }
    ...
    async setApiKey(provider, key) {
        ...
        // If this provider has LLM models and no model is selected, select the first one
        if (PROVIDERS[provider] && PROVIDERS[provider].llmModels.length > 0 && !existingSettings.selected_llm_model) {
            existingSettings.selected_llm_model = PROVIDERS[provider].llmModels[0];
        }
        
        // If this provider has STT models and no model is selected, select the first one  
        if (PROVIDERS[provider] && PROVIDERS[provider].sttModels.length > 0 && !existingSettings.selected_stt_model) {
            existingSettings.selected_stt_model = PROVIDERS[provider].sttModels[0];
        }
        
        await providerSettingsRepository.upsert(provider, { ...existingSettings, api_key: finalKey });
        
        // Auto-select this provider as active if it has models and API key
        if (finalKey && PROVIDERS[provider]) {
            if (PROVIDERS[provider].llmModels.length > 0) {
                await providerSettingsRepository.setActiveProvider(provider, 'llm');
            }
            if (PROVIDERS[provider].sttModels.length > 0) {
                await providerSettingsRepository.setActiveProvider(provider, 'stt');
            }
        }
        
        // Key was added/changed, check if we can auto-select models for this provider
    ...
    isLoggedInWithFirebase() {
        return false; // Firebase authentication removed - always return false
    }
