...
class DatabaseInitializer {
    constructor() {
        ...
        //   macOS: ~/Library/Application Support/OA-Coder/oacoder.db
        //   Windows: %APPDATA%\OA-Coder\oacoder.db
        this.dbPath = path.join(userDataPath, 'oacoder.db');
        this.dataDir = userDataPath;

        // The original DB path (read-only location in the package)
        this.sourceDbPath = app.isPackaged
            ? path.join(process.resourcesPath, 'data', 'oacoder.db')
            : path.join(app.getAppPath(), 'data', 'oacoder.db');
    }
