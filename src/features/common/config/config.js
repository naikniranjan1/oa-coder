...
class Config {
    constructor() {
        ...
        this.defaults = {
            apiUrl: process.env.OACODER_API_URL || 'http://localhost:9001',
            apiTimeout: 10000,
    ...
    loadEnvironmentConfig() {
        if (process.env.OACODER_API_URL) {
            this.config.apiUrl = process.env.OACODER_API_URL;
            console.log(`[Config] API URL from env: ${this.config.apiUrl}`);
        }
        
        if (process.env.OACODER_API_TIMEOUT) {
            this.config.apiTimeout = parseInt(process.env.OACODER_API_TIMEOUT);
        }
        
        if (process.env.OACODER_ENABLE_JWT) {
            this.config.enableJWT = process.env.OACODER_ENABLE_JWT === 'true';
        }
        
        if (process.env.OACODER_CACHE_TIMEOUT) {
            this.config.cacheTimeout = parseInt(process.env.OACODER_CACHE_TIMEOUT);
        }
        
        if (process.env.OACODER_LOG_LEVEL) {
            this.config.logLevel = process.env.OACODER_LOG_LEVEL;
        }
        
        if (process.env.OACODER_DEBUG) {
            this.config.enableDebugLogging = process.env.OACODER_DEBUG === 'true';
        }
    ...
    getUserConfigPath() {
        const configDir = path.join(os.homedir(), '.oacoder');
