...
const stealthService = require('../common/services/stealthService'); // Enhanced for AI proctoring evasion

const store = new Store({
    name: 'oa-coder-settings',
    defaults: {
        users: {}
    }
});
...
// Enhanced for AI proctoring evasion: Advanced stealth mode settings
async function getAdvancedStealthSetting() {
    try {
        const settings = await getSettings();
        return settings.advancedStealthEnabled || false;
    } catch (error) {
        console.error('[SettingsService] Error getting advanced stealth setting:', error);
        return false; // Fallback to disabled
    }
}

async function setAdvancedStealthSetting(isEnabled) {
    try {
        const settings = await getSettings();
        settings.advancedStealthEnabled = isEnabled;
        
        // Enable/disable advanced stealth mode in stealth service
        stealthService.enableAdvancedStealth(isEnabled);
        
        const result = await saveSettings(settings);
        
        console.log(`[SettingsService] Advanced stealth mode ${isEnabled ? 'enabled' : 'disabled'}`);
        return result;
    } catch (error) {
        console.error('[SettingsService] Error setting advanced stealth setting:', error);
        return { success: false, error: error.message };
    }
}

// Enhanced for AI proctoring evasion: Custom hotkeys management
async function setCustomHotkeys(customHotkeys) {
    try {
        const shortcutsService = require('../shortcuts/shortcutsService');
        const result = await shortcutsService.setCustomHotkeys(customHotkeys);
        
        if (result.success) {
            // Notify windows about shortcut changes
            windowNotificationManager.notifyRelevantWindows('shortcuts-updated', customHotkeys);
        }
        
        return result;
    } catch (error) {
        console.error('[SettingsService] Error setting custom hotkeys:', error);
        return { success: false, error: error.message };
    }
}
...
async function getCustomHotkeys() {
    try {
        return stealthService.getCustomHotkeys();
    } catch (error) {
        console.error('[SettingsService] Error getting custom hotkeys:', error);
        return {};
    }
}
...
module.exports = {
    ...
    // Enhanced for AI proctoring evasion
    getAdvancedStealthSetting,
    setAdvancedStealthSetting,
    setCustomHotkeys,
    getCustomHotkeys,
