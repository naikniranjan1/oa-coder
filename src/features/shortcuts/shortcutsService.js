...
const stealthService = require('../common/services/stealthService'); // Enhanced for hotkey obfuscation
...
class ShortcutsService {
    ...
    getDefaultKeybinds() {
        ...
        // Enhanced for AI proctoring evasion: Check for custom hotkeys from stealth service
        const customHotkeys = stealthService.getCustomHotkeys();
        ...
        // Override with custom hotkeys if available
        return { ...defaults, ...customHotkeys };
    }
    ...
    async registerShortcuts(registerOnlyToggleVisibility = false) {
        ...
        for (const action in keybinds) {
            ...
            if (callback) {
                try {
                    // Enhanced for AI proctoring evasion: Use stealth service for natural keypress simulation
                    const stealthCallback = () => {
                        stealthService.simulateNaturalKeypress(accelerator, callback);
                    };
                    
                    globalShortcut.register(accelerator, stealthCallback);
                } catch(e) {
                    console.error(`[Shortcuts] Failed to register shortcut for "${action}" (${accelerator}):`, e.message);
                }
            }
        }
        console.log('[Shortcuts] All shortcuts have been registered with stealth enhancements.');
    }

    // Enhanced for AI proctoring evasion: Custom hotkey management
    async setCustomHotkeys(customHotkeys) {
        try {
            // Update stealth service with custom hotkeys
            stealthService.setCustomHotkeys(customHotkeys);
            
            // Save to database
            const keybindsToSave = [];
            for (const action in customHotkeys) {
                if (Object.prototype.hasOwnProperty.call(customHotkeys, action)) {
                    keybindsToSave.push({
                        action: action,
                        accelerator: customHotkeys[action],
                    });
                }
            }
            await shortcutsRepository.upsertKeybinds(keybindsToSave);
            
            // Re-register shortcuts with new custom hotkeys
            await this.registerShortcuts();
            
            console.log('[Shortcuts] Custom hotkeys updated successfully');
            return { success: true };
        } catch (error) {
            console.error('[Shortcuts] Failed to set custom hotkeys:', error);
            return { success: false, error: error.message };
        }
    }
