const { BrowserWindow, globalShortcut, screen, app, shell, Tray } = require('electron');
...
const stealthService = require('../features/common/services/stealthService');
...
const openLoginPage = () => {
    ...
    console.log('Open login page - web removed');
};
...
function createFeatureWindows(header, namesToCreate) {
    ...
    const createFeatureWindow = (name) => {
        ...
        // Enhanced for AI proctoring evasion: Apply stealth web preferences to all windows
        const stealthWebPrefs = stealthService.getWebPreferencesForStealth();
        const enhancedChildOptions = {
            ...commonChildOptions,
            webPreferences: {
        ...
        switch (name) {
            case 'listen': {
                const listen = new BrowserWindow({
                    ...enhancedChildOptions, width:400,minWidth:400,maxWidth:900,
                    maxHeight:900,
                });
            ...
            // ask
            case 'ask': {
                const ask = new BrowserWindow({ ...enhancedChildOptions, width:600 });
            ...
            // settings
            case 'settings': {
                const settings = new BrowserWindow({ ...enhancedChildOptions, width:240, maxHeight:400, parent:undefined });
...
function createWindows() {
    // Task 5: Fix Linux system tray visibility - Disable system tray for Linux undetectability
    if (process.platform === 'linux') {
        // Explicitly prevent any tray icon creation on Linux
        try {
            if (Tray && typeof Tray.setIgnore === 'function') {
                Tray.setIgnore(true);
            }
        } catch (error) {
            console.log('[WindowManager] Tray disabled for Linux undetectability');
        }
    }
    ...
    // Enhanced for AI proctoring evasion: Get stealth web preferences
    const stealthWebPrefs = stealthService.getWebPreferencesForStealth();
