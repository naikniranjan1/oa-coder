...
export class SettingsView extends LitElement {
    ...
    static properties = {
        ...
        advancedStealthEnabled: { type: Boolean, state: true },
        advancedStealthLoading: { type: Boolean, state: true },
    ...
    constructor() {
        ...
        // Enhanced stealth properties
        this.advancedStealthEnabled = false;
        this.advancedStealthLoading = false;
    ...
    async loadAdvancedStealthSetting() {
        if (!window.api) return;
        this.advancedStealthLoading = true;
        try {
            const enabled = await window.api.settingsView.getAdvancedStealth();
            this.advancedStealthEnabled = enabled;
            console.log('Advanced stealth setting loaded:', enabled);
        } catch (e) {
            console.error('Error loading advanced stealth setting:', e);
            this.advancedStealthEnabled = false; // fallback
        }
        this.advancedStealthLoading = false;
        this.requestUpdate();
    }

    async handleToggleAdvancedStealth() {
        if (!window.api || this.advancedStealthLoading) return;
        this.advancedStealthLoading = true;
        this.requestUpdate();
        try {
            const newValue = !this.advancedStealthEnabled;
            const result = await window.api.settingsView.setAdvancedStealth(newValue);
            if (result && result.success) {
                this.advancedStealthEnabled = newValue;
            } else {
                console.error('Failed to update advanced stealth setting');
            }
        } catch (e) {
            console.error('Error toggling advanced stealth:', e);
        }
        this.advancedStealthLoading = false;
        this.requestUpdate();
    }
    ...
    connectedCallback() {
        ...
        this.loadAdvancedStealthSetting();
        ...
        return html`
            <div class="settings-container">
                <div class="header-section">
                    <div>
                        <h1 class="app-title">OA Coder</h1>
                    </div>
                ...
                <div class="buttons-section">
                    ...
                    <button class="settings-button full-width" @click=${this.handleToggleAdvancedStealth} ?disabled=${this.advancedStealthLoading}>
                        <span>Advanced Stealth Mode: ${this.advancedStealthEnabled ? 'On' : 'Off'}</span>
                    </button>
