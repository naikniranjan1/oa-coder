...
export class OACoderApp extends LitElement {
    static styles = css`
        :host {
            display: block;
            width: 100%;
    ...
    connectedCallback() {
        ...
        if (window.api) {
            window.api.oaCoderApp.onClickThroughToggled((_, isEnabled) => {
    ...
    disconnectedCallback() {
        ...
        if (window.api) {
            window.api.oaCoderApp.removeAllClickThroughListeners();
        }
    }
...
customElements.define('oa-coder-app', OACoderApp);
